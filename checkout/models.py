...
class OrderLineItem(models.Model):
    ...
    product_weight = models.CharField(max_length=10, null=True, blank=True) 
    ...
    def __str__(self):
        # Include weight in the string only if it exists
        if self.product_weight:
            return f'{self.product.name} ({self.product_weight}) on order {self.order.order_number}'
